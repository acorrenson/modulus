module Make(N: Logic.LIA_NUM) = struct
  open Logic.Make(N)
  open Core.Make(N)
  module Scripts = Scripts.Make(N)
  module Model = Model.Make(N)
  module Sexp = Sexp.Make(N)
  ...
  let do_get_model ctx =
    match ctx.state with
    | Sat_mode ->
      let vars = List.fold_left VSet.union VSet.empty (List.map vars ctx.stack) in
      VSet.iter (fun x ->
        match ctx.model with
        | SAT m ->
          Printf.eprintf "(%s %s)\n" x (Option.get (m x) |> N.to_string)
        | UNKNOWN -> ()
        | UNSAT -> assert false
      ) vars;
